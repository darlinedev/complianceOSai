...
export default function LoginPage() {
  ...
  const handleDemoMode = () => {
    setDemoLoading(true);
    setErrorMessage(null);
    // Simulate connection for the visual wow factor
    setTimeout(() => {
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-[420px] bg-slate-900/60 backdrop-blur-xl border border-slate-800 p-8 rounded-2xl shadow-2xl flex flex-col gap-6 relative z-10 animate-in fade-in zoom-in-95 duration-300">
        ...
        <button
          ...
          disabled={loading || demoLoading}
          className="w-full flex items-center justify-center gap-2 py-2.5 bg-slate-950 hover:bg-slate-900 disabled:bg-slate-800 text-slate-300 border border-slate-800 hover:border-slate-700 rounded-lg text-xs font-bold transition-all"
        >
          {demoLoading ? (
          ...
          {demoLoading ? "Lancement de la Démo..." : "Entrer en mode Démo (Simulé)"}
        </button>
